...
class Chat:
    def __init__(self, supabase, math_agent: MathAgent):
        self.supabase: ChatsSupabase = supabase
    ...
    def sync_error_generator(self, error, status_code):
    ...
    def new_chat(self, user_id:str, request: NewChatRequest, callback, auth_token):
        ...
        try:
            ...
            columns = dict()
            if upload_question_request.image_file:
                # image_bytes = upload_question_request.image_file.read()
                image_bytes = asyncio.run(upload_question_request.image_file.read())
                (
                ...
                parse_result = self.math_agent.parse_question(
                    columns[ChatColumn.IMAGE_STR], upload_question_request.prompt
                )
                ...
                columns[ChatColumn.TEXT_PROMPT] = upload_question_request.prompt
                wolfram_query = search(parse_result, "wolfram_query")
                if wolfram_query:
                    columns[ChatColumn.WOLFRAM_QUERY] = wolfram_query
            elif upload_question_request.prompt:
                columns[ChatColumn.QUESTION] = upload_question_request.prompt
                columns[ChatColumn.TEXT_PROMPT] = upload_question_request.prompt
            else:
                raise HTTPException(
                    status_code=500,
            ...
            columns[ChatColumn.LEARNER_MODE] = upload_question_request.mode == Mode.LEARNER
            data, count = self.supabase.update_chat_columns_by_id(
                upload_question_request.chat_id, columns, auth_token
            )
    ...
    # async def new_chat(self, request: NewChatRequest, creditService:CreditsSupabase, auth_token):
    #     chat_id = self.supabase.create_empty_chat(
